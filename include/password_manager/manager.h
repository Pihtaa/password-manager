...
private:
    ...
    std::unique_ptr<IMultiVaultStorage> m_vaults_storage;
...
public:
    PasswordManagerApp
        (std::unique_ptr<IMasterStorage> master_storage,
    ...
    bool check_master_password_strength(const secure_string& password) const;
    void put_master_password(const secure_string& password);
    void rederive_master_key(const secure_string& password);
    bool setup_master_password(const secure_string& password);


    bool login(const secure_string& password);
    bool is_logged_in() const;
    bool verify_master_password(const secure_string& password) const;
